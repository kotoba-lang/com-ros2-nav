(ns ros2_nav.main
  "Kotodama WASM entrypoint for the Ros2_nav clean-room actor (L4) — Clojure port.
  ...
  facts (`ros2_nav.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ros2_nav")
(def tier "L4")
...
(def entity-specs
  [{:entity "GoalStatus"   :plural "goalstatuses"   :id-prefix "ros2nav_goa"
    :fields [:goalId :stamp :status]              :required [:stamp :status]
    :coerce {:status :int}                       :refs {}}
   {:entity "NavSatFix"      :plural "navsatfixes"      :id-prefix "ros2nav_nav"
    :fields [:latitude :longitude :altitude :status :service :positionCovarianceType] :required [:latitude :longitude]
    :coerce {:latitude :float :longitude :float :altitude :float :status :int :service :int :positionCovarianceType :int} :refs {}}
   {:entity "BatteryState"     :plural "batterystates"     :id-prefix "ros2nav_bat"
    :fields [:voltage :temperature :current :charge :percentage :powerSupplyStatus :powerSupplyHealth :powerSupplyTechnology :present]        :required [:voltage :temperature]
    :coerce {:voltage :float :temperature :float :current :float :charge :float :percentage :float :powerSupplyStatus :int :powerSupplyHealth :int :powerSupplyTechnology :int :present :bool} :refs {}}
   {:entity "Odometry" :plural "odometries" :id-prefix "ros2nav_odo"
    :fields [:childFrameId :poseX :poseY :poseZ :twistLinearX :twistLinearY :twistLinearZ :twistAngularX :twistAngularY :twistAngularZ]          :required [:poseX :poseY]
    :coerce {:poseX :float :poseY :float :poseZ :float :twistLinearX :float :twistLinearY :float :twistLinearZ :float :twistAngularX :float :twistAngularY :float :twistAngularZ :float} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ros2_nav.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ros2_nav-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ros2_nav-compat/ready)
